    ...
    public partial class FormRequest : FormAbstractClass
    {
        ...
        private void btnInsert_Click(object sender, EventArgs e)
        {
            ...
            using (var transaction = dbContext.Database.BeginTransaction())
            {
                try
                {
                    // Создание нового потенциального абонента
                    ...
                    // Добавление нового потенциального абонента в базу данных
                    dbContext.PotentialSubscribers.Add(potentialSubscriber);
                    dbContext.SaveChanges(); // Сохранение изменений в БД для получения ID потенциального абонента

                    // Получение ID только что созданного потенциального абонента
                    ...
                    Request request = new Request
                    {
                        ...
                        TariffId = (int)formNewRequest.comboBoxTariff.SelectedValue, // Получение ID выбранного тарифа
                        EquipmentId = formNewRequest.checkBoxEquipment.Enabled ? (int?)formNewRequest.comboBoxEquipment.SelectedValue : null, // Получение ID выбранного оборудования
                    ...
                    dbContext.SaveChanges(); // Сохранение изменений в БД для добавления заявки
                ...
                {
                    // Обработка исключения, возникающего при выполнении операций в базе данных
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (dataGridViewRequest.SelectedRows.Count == 1)
            {
                ...
                if (requestToUpdate != null)
                {
                    ...
                    {
                        // Если индекс не найден, выберается первый элемент в комбо-боксе
                    ...
                    {
                        formNewRequest.comboBoxEquipment.SelectedIndex = 0; // Выбор первого элемента в коллекции
                        formNewRequest.checkBoxEquipment.Checked = true;
                    }

                    // Установка значений даты и времени в элементы управления
                    ...
                    {
                        formNewRequest.comboBoxStatus.SelectedIndex = 0; // Выбор первого элемента в коллекции по умолчанию
                    }
                    ...
                    using (var transaction = dbContext.Database.BeginTransaction())
                    {
                        try
                        {
                            // Обновляем данные заявки из элементов формы
                            ...
                            dbContext.SaveChanges(); // Обновление выбранной заявки в базе данных
                            transaction.Commit(); // Фиксация транзакции
