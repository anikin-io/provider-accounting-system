        ...
        private void btnInsert_Click(object sender, EventArgs e)
        {
            ...
            using (var transaction = dbContext.Database.BeginTransaction())
            {
                try
                {
                    ...
                    dbContext.SaveChanges(); // Выполнение сохранения в базу данных
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridViewTariff.SelectedRows.Count == 1)
            {
                ...
                if (tariffToUpdate != null)
                {
                    ...
                    {
                        // Установка выбранного элемента в ComboBox по значению tariffToUpdate.PaymentType
                        formNewTariff.comboBoxType.SelectedItem = tariffToUpdate.PaymentType.ToString();
                    }
                    else
                    {
                        // Если значение отсутствует в коллекции, можно установить значение по умолчанию
                    ...
                    using (var transaction = dbContext.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            dbContext.SaveChanges(); // Обновление выбранного тарифа в базе данных
                            transaction.Commit(); // Фиксация транзакции
