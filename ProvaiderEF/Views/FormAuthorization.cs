    ...
    public partial class FormAuthorization : FormAbstractClass
    {
        ...
        private void AttemptAuthorization()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                : string.Empty; // Если значение NULL, устанавливаем пустую строку
            }

            if (isAuthenticated)
            {
                // В зависимости от значения "ТипСотрудника" открываем соответствующий личный кабинет
                switch (EmployeeType)
                {
                    ...
                    case "Мастер":
                        this.Hide();
                        FormMenuMaster formMenuMaster = new FormMenuMaster(employeeId, EmployeeFirstName, EmployeeMiddleName); // Передаем employeeId в конструктор
                        formMenuMaster.Show();
                        break;
